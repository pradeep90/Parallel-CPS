...
#+begin_src java
  task fib(int k, Activation later) {
      if (k <= 2) {
          ...
          schedule(left, new ContinuationLeft(k, right));
          schedule(right, new ContinuationRight(k, sum));
          schedule(sum, new ContinuationSum(then, left, right, later));
          left→right; //inserted by naive translation
          right→sum;
          sum→later;
      }
  ...
  class ContinuationLeft extends Continuation {
      ...
      public ContinuationLeft(int k, Activation later){
          this.k = k;
          this.later = later;
      }
  ...
  class ContinuationSum extends Continuation {
      ...
      public ContinuationLeft(Activation then, Activation left, Activation right, Activation later){
          this.then = then;
          this.left = left;
          this.right = right;
          this.later = later;
